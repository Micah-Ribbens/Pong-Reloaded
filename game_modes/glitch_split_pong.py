...
from base_pong.utility_functions import deepcopy
...
class SplitPong(PongType):
    ...
    def increase_ball_size(ball):
        ball.length += (SplitPong.base_ball_length * 1)
        ball.height += (SplitPong.base_ball_length * 1)

    def ball_is_ready_to_split(ball):
        return ball.length >= SplitPong.base_ball_length * 2

    def split(ball: Ball, new_balls):
        ...
        new_ball.x_coordinate += distance_change
