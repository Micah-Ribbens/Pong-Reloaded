...
class GravityPong(GameMode):
    def ball_collisions(ball, paddle1, paddle2):
        vertex_increase = 1.05
        max_vertex = screen_height - ball.height

        ball_has_collided = CollisionsFinder.is_collision(ball, paddle1) or CollisionsFinder.is_collision(ball, paddle2)
    ...
    def ball_gravity(ball):
        ...
        GravityPong.time_since_ground += VelocityCalculator.time
        y_change = PhysicsEngine.distance_change(GravityPong.vertex, .6, GravityPong.time_since_ground)
